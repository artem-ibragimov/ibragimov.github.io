		...
		article: {
			css: {
				'css-grid--gayd-po-reshyotke': {
                  ...
                  Эта статья охватывает основы, продвинутые техники и практические советы для адаптивной верстки`,
					chapters: [
						[
							'CSS Grid Layout - что это такое?',
							`Grid - изумителен, ведь позволяет создавать сложные и гибкие макеты. 
                     Одновременно с этим Grid не прост, и мне потребовалось время, чтобы освоить его.`
						],
						[
							'Поддержка браузерами',
							`Grid - современный инструмент верски макетов, поддерживается с 2017 года почти 98 % браузеров.
                     Потрясающая поддержка, лучше только у Flexbox`
						],
						[
							'Модель Grid',
							`CSS предоставляет несколько алгоритмов разметки макетов:
                     <ul>
                     ...
                     Это необычно, ведь в других макетах структура определяется дочерними DOM элементами.`
						],
						[
							'Grid Макет',
							`По-умолчанию создается один столбец и дочерние элементы располагаются на каждой строке.
                     Это неявная сетка, т.к мы не задали структуру.
                     Высота определяется дочерними элементами, по правилам Flow макета,
